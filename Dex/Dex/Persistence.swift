...
struct PersistenceController {
    ...
    // for kornking in the pokemonDetail view we need a sample pokemon
    static var previewPokemon : Pokemon {
        let context = PersistenceController.preview.container.viewContext
        let fetchRequest: NSFetchRequest<Pokemon> = Pokemon.fetchRequest()
        fetchRequest.fetchLimit = 1 // only one for the preview 
    ...
    static let preview: PersistenceController = {
        ...
        newPokemon.id = 1
        newPokemon.name = "bulbasur" // optional by default, needs to be unwrapped, also types and Urls
    ...
    init(inMemory: Bool = false) {
        ...
        container.viewContext.mergePolicy = NSMergePolicy.mergeByPropertyStoreTrump // Keep the one already stored in db
